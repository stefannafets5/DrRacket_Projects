...
(define (merge ph1 ph2)
  (cond
    ((null? ph1) ph2)
    ((null? ph2) ph1)
    (else (if (> (ph-root ph2) (ph-root ph1))
...
(define (merge-two-by-two phs acc)
  (cond
    ((null? phs) acc)
    ((null? (cdr phs)) (merge acc (make_list (car phs))))
    (else (merge-two-by-two (cddr phs) (merge acc (merge (make_list (car phs)) (make_list (cadr phs))))))
    )
  )

(define (two-pass-merge-LR phs)
   (merge-two-by-two phs '())
  )

(define (make_list lst)
...
(define (tournament-merge phs)
  (cond
    ((null? phs) '())
    ((null? (cdr phs)) (car phs))
    (else (tournament-merge (merge2 phs)))))
...
(define (merge2 phs)
  (cond
    ...
    (else (append
           (list (merge (car phs) (car (cdr phs))))
           (merge2 (cdr (cdr phs)))))))
...
(define (ph-del-root ph)
  (if(null? ph) #f (two-pass-merge-LR (cdr ph))))
