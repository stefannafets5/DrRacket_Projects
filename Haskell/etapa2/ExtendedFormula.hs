...
eliminate literal = Map.mapKeys (Set.delete $ complement literal) . Map.filterWithKey (const . not . Set.member literal)
...
firstPureLiteral formula = promote (\simple_formula -> Set.foldr (\literal acc ->
    if isPureLiteral literal simple_formula
        then Just literal
            else acc)
    Nothing (formulaLiterals simple_formula)) formula
...
firstUnitClause = Map.foldrWithKey (\k v acc ->
  if isUnitClause k
    then Just (Set.findMin k, v)
      else acc)
  Nothing
...
maybe.
-}
process select makeAction modifyFormula history =
  case history of
    ((lastAct, current) : _) ->
      case select current of
        Nothing   -> history  -- fără modificări
        Just info ->
          let action   = makeAction info
              newForm  = modifyFormula info current
              result   = (action, newForm) : history
          in case action of
               Decide _ -> result
               _        -> process select makeAction modifyFormula result
    [] -> []
...
decide = process pickLiteral Decide eliminate
  where
    pickLiteral formula =
      let allLits = foldr Set.union Set.empty (Map.keys formula)
      in Set.lookupMin allLits
{-
*** TODO ***
...
processPureLiterals = process firstPureLiteral Pure eliminate
...
processUnitClauses =
  process extractUnit makeUnitAction applyElimination
  where
    extractUnit = firstUnitClause
    makeUnitAction (lit, cls) = Unit lit cls
    applyElimination (lit, _) = eliminate lit
...
backtrackToUnitClause clause history =
  let reversed = reverse history
      isUnitAfter c = isUnitClause . Set.delete (complement cLit)
        where cLit = Set.findMin c
      -- căutăm prima formulă unde clauza devine unitară
      maybeIndex = 
        let cLit = Set.findMin clause
            isUnitHere (_, ef) = isUnitClause (Set.delete (complement cLit) clause)
                                              && Set.member clause (baseFormula ef)
        in findIndex isUnitHere reversed
  in case maybeIndex of
       Nothing -> history -- nu s-a găsit niciun punct unde clauza devine unitară
       Just i  ->
         let (past, future) = splitAt (i + 1) reversed
             -- reconstrucția formulei cu clauza adăugată
             (act, ef) = last past
             newEF = Map.insert clause clause ef
             -- refacem acțiunile din viitor
             replayed = tail $ reverse $ foldl (\h (a, _) -> replayAction a h) [(NOP, newEF)] (reverse future)
         in replayed
