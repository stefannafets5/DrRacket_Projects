...
interpretations = map Set.fromList . sequence . map (\v -> [v, -v]) . Set.toList
...
1. Cum contribuie evaluarea leneșă la explorarea eficientă a interpretărilor?
  ...
  Daca gasesti ce cautai nu mai trebuie sa continui evaluarea. Si nu mai faci
  efort de calcul degeaba.
...
2. Ce s-ar fi întâmplat dacă interpretations întorcea o mulțime (Set) de 
   ...
   Daca ar intoarce un set, ordinea nu ar fi garantata, pe cand lista e ordonata.
...
isSatisfiable formula =
  let vars = formulaVariables formula
      allInterps = interpretations vars
  in any (`satisfiesFormula` formula) allInterps
